...
for file in default.css default.js error.png passed.png run.rb template.html; do
    curl https://raw.githubusercontent.com/seoh/dandy/master/src/$file -o ~/.dandy/$file
done

echo "워크플로우 파일을 설치합니다."

# 압축한 워크플로우 파일을 해제하여 서비스 디렉토리에 옮긴다.
curl https://raw.githubusercontent.com/seoh/dandy/master/build/dandy.zip -o ~/.dandy/dandy.zip
